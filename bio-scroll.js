document.addEventListener('DOMContentLoaded', () => {
    const body = document.querySelector('body.body-bio');
    if (!body) return; 
    ...
    const backgroundFadeOutPoint = transitionDuration * 2 / 3;
    ...
    function updateVisuals() {
        ...
        // --- Header Fade Out ---
        if (header) {
            const headerProgress = Math.min(1, scrollY / headerFadeOutPoint);
        ...
        if (scrollY <= transitionDuration) {
            ...
            // Middle section fade (last third)
            ...
            if (footer) footer.style.opacity = 1; // Keep footer visible for arrow
        }
        // --- Phase 2: Middle Section -> Black -> Final Image ---
            ...
            // Middle section fade out (first third)
            const msProgress = Math.min(1, phaseProgress * 3);
            middleSection.style.opacity = 1 - easeInOutCubic(msProgress);
            ...
            if (footer) footer.style.opacity = 1 - easeInOutCubic(msProgress); 
        }
    ...
    const contentWrapper = document.querySelector('#middle-section');
