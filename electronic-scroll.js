document.addEventListener('DOMContentLoaded', () => {
    const body = document.querySelector('body.body-electronic');
    if (!body) return; 

    const background = document.getElementById('electronic-background');
    const middleSection = document.getElementById('middle-section');
    const videoContainer = document.getElementById('video-container');
    const header = document.querySelector('.electronic-title');
    const sidebar = document.getElementById('sidebar-placeholder');
    const footer = document.querySelector('.footer');
    const scrollArrow = document.getElementById('scroll-arrow');
    const scrollArrowUp = document.getElementById('scroll-arrow-up');
    const bandcampWrapper = document.querySelector('.bandcamp-player-wrapper');
    ...
    let isBandcampHidden = false;

    // YouTube API integration to pause Bandcamp when video starts
    window.onYouTubeIframeAPIReady = () => {
        new YT.Player('youtube-player', {
            events: {
                'onStateChange': (event) => {
                    if (event.data === YT.PlayerState.PLAYING) {
                        const bcIframe = document.getElementById('bandcamp-iframe');
                        if (bcIframe) {
                            bcIframe.src = bcIframe.src; // Pause by reloading
                        }
                    }
                }
            }
    ...
    function updateVisuals() {
        // --- Bandcamp Shrink & Hide Logic ---
        if (scrollY > 50) {
            if (bandcampWrapper) bandcampWrapper.classList.add('shrunk');
        } else {
            if (bandcampWrapper) bandcampWrapper.classList.remove('shrunk');
        }

        if (scrollY > transitionDuration) {
            if (!isBandcampHidden && bandcampWrapper) {
                isBandcampHidden = true;
                bandcampWrapper.style.opacity = '0';
                bandcampWrapper.style.pointerEvents = 'none';
            }
        } else {
            if (isBandcampHidden && bandcampWrapper) {
                isBandcampHidden = false;
                bandcampWrapper.style.opacity = '1';
                bandcampWrapper.style.pointerEvents = 'auto';
            }
        }
        ...
        // --- Phase 2: Middle Section -> Black -> Video ---
            ...
            const videoProgress = Math.max(0, (phaseProgress - 2/3) * 3);
            videoContainer.style.opacity = easeInOutCubic(Math.min(1, videoProgress));
        ...
        const videoOpacity = parseFloat(videoContainer.style.opacity) || 0;
        const iframe = videoContainer.querySelector('iframe');
        if (videoOpacity > 0.95) {
    ...
    window.addEventListener('keydown', (event) => {
        ...
        if (event.key === 'PageDown' || event.key === 'ArrowDown') {
            if (middleSection.style.pointerEvents === 'auto' && contentWrapper && (contentWrapper.scrollTop < contentWrapper.scrollHeight - contentWrapper.clientHeight)) {
            ...
            if (middleSection.style.pointerEvents === 'auto' && contentWrapper && contentWrapper.scrollTop > 0) {
                return; // Let native scroll happen
            }
            event.preventDefault();
            let targetScrollY = 0;
